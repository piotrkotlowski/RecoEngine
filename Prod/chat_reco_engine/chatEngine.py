...
from .pydanticHelper import productQuery,isHistoryConnected, conversationType,conversationTypeEnum, QueryContext
import json 
import chromadb
...
class chatRecommendationSystem:
    ...
    def return_response(self, user_id: int, user_text: str) -> str:
        ...
        if convType == conversationTypeEnum.recommendation:
            ...
            if mode == "new":
                products = self._get_products(user_text)
                if len(products) == 0:
    ...
    def _get_products(self, user_text: str, n_results: int = 3):
        """"
